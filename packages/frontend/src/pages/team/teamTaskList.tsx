...
export function TeamTaskList() {
  ...
  const { data: teamData } = useTeamOne(id);
  ...
  // Add polling effect
  useEffect(() => {
    // Don't poll if any user interaction is in progress
    if (isCreateDrawerOpen || isEditDrawerOpen || isDeleteDialogOpen || isCreateBucketDialogOpen) {
      return;
    }
    ...
    <Box>
      <SectionHeader
        breadcrumbs={[
          ...
          { label: teamData.name, to: `/teams/${id}` },
          { label: 'Team tasks' }
        ]}
      >
